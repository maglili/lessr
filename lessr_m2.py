...
class AttnReadout(nn.Module):
    ...
    def forward(self, g, feat, last_nodes):
        ...
        # feat_value = self.fc_value(feat)

        # print("feat.shape:", feat.shape)
        # print("feat_u.shape:", feat_u.shape)
        # print("feat_v.shape:", feat_v.shape)
        # print("feat_value.shape:", feat_value.shape)

        # print(th.cat((feat_u, feat_v), 1).shape)

        e = self.fc_e(th.tanh(feat_u + feat_v))
        # e = self.fc_e(th.tanh(self.W(th.cat((feat_u, feat_v), 1))))
        alpha = F.segment.segment_softmax(g.batch_num_nodes(), e)

        # feat_norm = feat * alpha
        feat_norm = self.fc_value(feat) * alpha
