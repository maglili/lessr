...
class EOPA(nn.Module):
    def __init__(
        ...
        self.tf_encoder_layer = nn.TransformerEncoderLayer(
            d_model=input_dim, nhead=8, dim_feedforward=512
        )

        # *****************numer of layers***********************
        self.tf_encoder = nn.TransformerEncoder(self.tf_encoder_layer, num_layers=1)

        # self.multihead_attn = nn.MultiheadAttention(embed_dim=input_dim, num_heads=32)

        # *****************pos encoding*************************
        self.pos_encoder = PositionalEncoding(d_model=input_dim, dropout=0)
    ...
    def reducer(self, nodes):
        ...
        hn = self.tf_encoder(m)
        # hn, _ = self.multihead_attn(m, m, m)
        hn = hn[-1, :, :]
        hn = hn.unsqueeze(0)
        # print("m.shape:", m.shape)
        # print("m.permute.shape:", m.shape)
        # print("hn.shape:", hn.shape)
        # print("hn [sliced]", hn.shape)
        # print("hn unsqueeze:", hn.shape)
